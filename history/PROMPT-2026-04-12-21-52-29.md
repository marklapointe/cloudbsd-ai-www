...
37. **Full Regression Testing & Stability**:
    - **Automated Regression Suite**: Successfully executed a full project-wide regression test involving 260 unit and integration tests with a 100% pass rate.
...
38. **User Profile Persistence & Synchronization**:
    - **Backend Persistence**: Added `theme` column to the `users` table and implemented `/api/users/profile` (GET/PUT) endpoints to store and retrieve user preferences.
    - **Theme Synchronization**: Updated `ThemeContext` to automatically synchronize the current theme with the backend on change and fetch the user's preferred theme upon application mount or login.
    - **Robust Frontend Testing**: Enhanced the `Login` component test suite with `data-testid` selectors and improved `i18next` isolation, ensuring reliable verification of authentication and preference application.
    - **Default Dark Mode**: Ensured that the application defaults to 'Dark' mode if no user setting is found in the database or local storage.
39. **Code Coverage & Quality**:
    - **Coverage Integration**: Integrated `@vitest/coverage-v8` into the test runner to track code coverage across the entire project.
    - **High Coverage Standard**: Added comprehensive unit tests for core backend functionalities (Resource management, User profiles) and frontend state management (ThemeContext), targeting a 90%+ coverage for critical modules.
    - **Test Isolation**: Improved backend test isolation by ensuring each test file resets its own database state (e.g., license limits) to prevent interference during parallel execution.
40. **UI Refinement - Native Dialog Removal**:
