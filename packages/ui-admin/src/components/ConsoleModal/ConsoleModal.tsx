...
import type { ConsoleModalProps } from './types';
...
export const ConsoleModal: React.FC<ConsoleModalProps> = ({ 
  ...
  useEffect(() => {
    if (isOpen && resourceType !== 'vms' && terminalRef.current && resource && socket) {
      // Initialize xterm.js
      ...
      const handleOutput = (data: { resourceId: string | number, output: string }) => {
  ...
  }, [isOpen, resource, resourceType, socket, t]);
