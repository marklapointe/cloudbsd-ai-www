    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          ...
          className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-black uppercase text-xs tracking-widest transition-all duration-300 shadow-lg active:scale-95 border-b-4 hover:border-b-0 hover:translate-y-[2px] ${
            showAddForm 
              ? 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-300 dark:border-slate-900 hover:bg-slate-300 dark:hover:bg-slate-700' 
              : 'bg-brand-600 hover:bg-brand-700 text-white border-brand-800 dark:border-brand-900 shadow-brand-500/20'
          }`}
        >
          <UserPlus size={18} />
      ...
      {showAddForm && (
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-xl border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-300">
          ...
          <form onSubmit={handleCreateUser} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 items-end">
            ...
            <button 
              type="submit"
              className="bg-brand-600 hover:bg-brand-700 text-white px-6 py-3.5 rounded-2xl font-black uppercase text-xs tracking-widest transition-all duration-300 shadow-lg shadow-brand-500/20 active:scale-95 border-b-4 border-brand-800 dark:border-brand-900 hover:border-b-0 hover:translate-y-[2px]"
            >
              {t('common.save')}
            </button>
          </form>
              ...
              <tr key={user.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-all duration-200">
                ...
                <td className="px-8 py-5 text-right">
                  <button 
                    ...
                    className={`p-2.5 rounded-xl transition-all duration-200 active:scale-90 border border-transparent shadow-sm ${
                      user.username === 'admin' 
                        ? 'text-slate-200 dark:text-slate-800 cursor-not-allowed' 
                        : 'text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 hover:border-red-100 dark:hover:border-red-500/20 hover:shadow'
                    }`}
